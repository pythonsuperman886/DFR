...
from utils import CutPasteNormal,add_defect,Cutout,cut_defect_paste
...
class NormalDataset(Dataset):
    ...
    def __init__(self, path, normalize=True):
        ...
        self.mask_transform = transforms.Compose([resize,gray,transforms.ToTensor()])
        self.cut_out = Cutout(10,20)
        self.cutpaste = CutPasteNormal()
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            ...
            # img = randoma(img)
            # mask = Image.new('RGB', (img.width, img.height), (0, 0, 0))
            # img_anormal,mask = self.cutpaste(img_anormal,mask)
            # img_anormal,mask = self.cutpaste(img_anormal,mask)
            ...
            # img_anormal,mask = self.cut_out(img_anormal)
            # img_anormal,mask = cut_defect_paste(img_anormal,mask)
            # img_anormal,mask = cut_defect_paste(img_anormal,mask)

            # img_anormal, mask = self.cut_paste(img_anormal, mask)
            # img_anormal, mask = self.cut_paste(img_anormal, mask)
            # img_anormal, mask = self.cut_paste(img_anormal, mask)
            # img_anormal, mask = self.cut_paste(img_anormal, mask)
            # img_anormal, mask = self.cut_paste(img_anormal, mask)
            ...
            img_anormal = self.transform(img_anormal)
            mask = self.mask_transform(mask)
        return img,img_anormal,mask
...
class Train_abnormal_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if img_name.split('/')[-2] == "good":
            ...
            if "gangsi" in self.img_files[idx]:
                # mask = Image.new('RGB', (img.width, img.height), (0, 0, 0))
                mask_path = img_name.replace("train", "ground_truth").split(".")[-2] + ".png"
                ...
                if "wine" in self.img_files[idx]:
                    ...
                    mask = Image.open(mask_path)
                    # mask = resize(mask, (256, 256))
                if "semiconductor" in self.img_files[idx]:
                    mask_path = img_name.replace("train", "ground_truth")#.split(".")[-2] + ".png"
                    mask_path = mask_path.replace("ori","label")
                    mask = Image.open(mask_path)
                    print("mask path: ",mask_path)
                    
                if "yinxian" in self.img_files[idx]:
                    mask_path = img_name.replace("train", "ground_truth")#.split(".")[-2] + ".png"
                    ...
                    mask_path = img_name.replace("train", "ground_truth").split(".")[-2] + "_mask.png"
                    # mask_path = img_name.replace("test", "ground_truth").split(".")[-2] + ".png"
                    # mask_path = img_name.replace("test", "ground_truth").split(".")[-2] + ".jpg"
    ...
    def _get_image_files(self, path, ext={'.jpg', '.png'}):
        ...
        for root, dirs, files in os.walk(path):
            ...
            for file in files:
                ...
                # if os.path.splitext(file)[1] in ext:  # and "good" not in root
                    images.append(os.path.join(root, file))
                    print('loading image files ' + root+'/'+file)
    ...
    def __getitem__(self, idx):
        ...
        if img_name.split('/')[-2] == "good":
            ...
            if "wine" in self.img_files[idx]:
                ...
                if "gangsi" in self.img_files[idx]:
                    # mask = np.random.randint(0,255,size=(256,256))
                    # mask = img
                    mask_path = img_name.replace("test", "ground_truth").split(".")[-2] + ".png"
                    mask = imread(mask_path, as_gray=True)
                    mask = resize(mask, (256, 256))
                    ...
                    if "yinxian" in self.img_files[idx]:
                        ...
                        # cv2.imshow("w",mask)
                        # cv2.waitKey(0)
                        # np.savetxt("./mask_path.txt", mask, fmt="%.2f")
